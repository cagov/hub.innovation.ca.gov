---
layout: single-column
title: Resources and references
parentid: Data
parentidlink: /data/
subparentid: IDEA guidebook
subparentidlink: /data/idea-guidebook/
description: Use these steps to develop a BUCP under the IDEA.
headerlabel: Guides and playbooks
headericon: book
---

<p class="text-lead">In this section you will find key templates and other information to execute and understand the IDEA process. These materials are linked throughout the guide.</p>

This section includes: 

* [IDEA agreement](/data/idea-guidebook/resources-references/agreement/)
* [List of signatories](/data/idea-guidebook/resources-references/list-signatories/)
* [BUCP template](/data/idea-guidebook/resources-references/bucp-template/)
* [Notice, email, and disclosure templates](/data/idea-guidebook/resources-references/notice-email-disclosure-templates/)

<div class="leftright-nav-container">
    <div class="left-nav"><a class="internal-link" href="/data/idea-guidebook/dispute-resolution-process/">Dispute resolution process</a></div>
    <div class="right-nav"><a class="internal-link" href="/data/idea-guidebook/resources-references/agreement/">IDEA agreement</a></div>
</div>
